...
/** Handles the messages incoming from users **/
...
ConnStatus handleNewConnection(Client *client, void *_wsserver) {
  char buffer[1024] = {0};
  i32 len =
      snprintf(buffer, 1023, "New client connected: %s: %d\n",
...
void handleDisconnect(Client *client, void *_wsserver) {
  char buffer[1024] = {0};
  i32 len =
      snprintf(buffer, 1023, "Client disconected: %s: %d\n",
               inet_ntoa(client->addr.sin_addr),
               htons(client->addr.sin_port));
...
int main(int argc, char *argv[]) {
  ...
  conInitServer(&skserver,(ServerConfig){.port = 8090, .max_connections = 10000});
